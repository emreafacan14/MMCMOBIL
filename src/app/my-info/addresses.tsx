...
import { addressTypeLabel, formatAddressSubtitle } from "@/components/my-info/shared";
...
export default function AddressesScreen() {
  ...
  const removeMutation = userAddressHooks.useRemove();
  ...
  const displayTitle = (item: UserAddress): string =>
    item.title ?? addressTypeLabel(item.addressType, t);
  ...
  const handleEditPress = (item: UserAddress) => {
    haptic("light");
    router.push(`/my-info/address-form?id=${item.id}`);
  };

  const handleDeleteRequest = (item: UserAddress) => {
    haptic("warning");
    confirmDialog.show({
  ...
  const handleDeleteConfirm = async (item: UserAddress) => {
    try {
  ...
  const renderRow = ({ item }: ListRenderItemInfo<UserAddress>): ReactElement => (
    ...
    if (items.length === 0) {
      return (
        <View className="flex-1 items-center justify-center px-5">
          <EmptyState
            icon={MapPin}
            title={t("myInfo.addresses.empty")}
            actionLabel={t("myInfo.addresses.add")}
