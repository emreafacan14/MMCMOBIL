...
import { emailTypeLabel } from "@/components/my-info/shared";
...
export default function EmailsScreen() {
  ...
  const removeMutation = userEmailHooks.useRemove();
  ...
  const displayTitle = (item: UserEmail): string =>
    item.title ?? emailTypeLabel(item.emailType, t);
  ...
  const handleEditPress = (item: UserEmail) => {
    haptic("light");
    router.push(`/my-info/email-form?id=${item.id}`);
  };

  const handleDeleteRequest = (item: UserEmail) => {
    haptic("warning");
    confirmDialog.show({
  ...
  const handleDeleteConfirm = async (item: UserEmail) => {
    try {
  ...
  const renderRow = ({ item }: ListRenderItemInfo<UserEmail>): ReactElement => (
    ...
    if (items.length === 0) {
      return (
        <View className="flex-1 items-center justify-center px-5">
          <EmptyState
            icon={Mail}
            title={t("myInfo.emails.empty")}
            actionLabel={t("myInfo.emails.add")}
